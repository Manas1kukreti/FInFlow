...
def get_string_columns(df: pd.DataFrame, requested_cols: Any) -> List[str]:
    ...
    # Treat object-dtype columns as string columns too. On pandas 2.x,
    # ``is_string_dtype`` returns False for an ``object`` column that contains
    # nulls, which is the normal representation for Python string data. This
    # keeps the explicit-column path consistent with the
    # ``__all_string_columns__`` branch above (``select_dtypes(['object',
    # 'string'])``) so operations like trim_whitespace still apply.
